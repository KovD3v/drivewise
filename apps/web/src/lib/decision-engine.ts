...
export type ScoredVehicle = { vehicle: Vehicle; score: number; why: string[]; tradeoffs: string[]; offer?: AdvisorOffer };
...
export type PreviewVehicle = { id: string; name: string; segment: string; score: number; vehicleType: VehicleTypeKey };
export function toPreview(s: ScoredVehicle): PreviewVehicle {
  return { id: s.vehicle.id, name: `${s.vehicle.brand} ${s.vehicle.model}`, segment: s.vehicle.category,
    score: s.score, vehicleType: s.vehicle.vehicle_type };
}
